...
from ._ipc_ring import IpcRing, RingFull
...
class CudaIPCConnector(OmniConnectorBase):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Ring control plane (P1.2). Default OFF: keep the per-transfer /dev/shm path
        ...
        self._ring: IpcRing | None = None  # sender: created at init; receiver: None
        self._opened_rings: dict[tuple[str, str], IpcRing] = {}  # receiver cache
    ...
    def _open_ring_receiver(self, from_stage, to_stage):
        ...
        if ring is None:
            try:
                ring = IpcRing.open(self._ring_name(from_stage, to_stage))
            except FileNotFoundError:
